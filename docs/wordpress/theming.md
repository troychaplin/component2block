...
The `output.themeable` flag in `c2b.config.json` controls how much freedom themes and editors have:

| Behavior | Locked (default) | Themeable |
|----------|-------------------|-----------|
| Config | `output.themeable: false` | `output.themeable: true` |
...
Set `output.themeable: true` in the library's `c2b.config.json` and re-run the generator. This produces `tokens.wp.css` which maps preset tokens to `--wp--preset--*` variables:
...
2. Verify `output.themeable: true` is set in `c2b.config.json` and you've re-run the generator
