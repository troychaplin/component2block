...
The generator produces a `clamp()` value matching WordPress/Gutenberg's fluid typography formula:
...
The formula scales linearly between the `min` value at a 320px viewport and the `max` value at the max viewport. The max viewport is resolved in this order:

1. Explicit `fluid.maxViewport` in config
2. `layout.wideSize` from your layout tokens (mirrors WordPress behavior)
3. `1600px` default

This means if you define `layout.wideSize` in your tokens, the fluid calculations automatically match what WordPress generates from the same `theme.json` — no extra configuration needed.
...
Fluid `min` and `max` accept `rem` or `em` units. Both values must use the same unit.
...
When any `fontSize` tokens are defined, the generator sets `typography.fluid` in theme.json with explicit viewport anchors so WordPress uses the exact same fluid calculation as the generated `tokens.css`.
...
Generated when any `fontFamily` entry has a `fontFace` array. Placed in the `output.srcDir` directory:
