...
import type { C2bConfig, C2bConfigInput, FluidConfig, TokenCategory, TokenEntry, TokenEntryInput, TokenGroup, TokenGroupInput, BaseStyleElementDef, BaseStylesConfig, BaseStylesSpacingPadding, FluidInput } from './types.js';
...
const CONFIG_KEYS = ['prefix', 'output', 'tokens', 'baseStyles', 'fluid'] as const;
...
function extractTokens(input: C2bConfigInput): Partial<Record<TokenCategory, TokenGroup>> {
  ...
  for (const [key, value] of Object.entries(source)) {
    ...
    if (!VALID_CATEGORIES.includes(internalCategory as TokenCategory)) {
      ...
      throw new Error(
        `Config error: Unknown token category "${key}". ` +
        (removed ? `${removed} ` : '') +
        `Valid categories: ${validInputCategories.filter(c => VALID_CATEGORIES.includes((INPUT_CATEGORY_MAP[c] ?? c) as TokenCategory)).join(', ')}`,
      );
    }
 ...
 */
const REMOVED_CATEGORIES: Record<string, string> = {
  mediaQuery:
    'The "mediaQuery" category was removed. Use "viewport" for the mobile and tablet ' +
...
const ROOT_PADDING_KEYS = ['x', 'y', 'top', 'right', 'bottom', 'left'] as const;
 ...
 */
export function resolveRootPadding(padding: BaseStylesSpacingPadding | undefined): BaseStylesSpacingPadding {
  if (!padding) return {};
...
function sharedValue(a: string | undefined, b: string | undefined): string | undefined {
 ...
 */
const VIEWPORT_KEYS = ['mobile', 'tablet'] as const;
 ...
 */
function parseLength(value: string | undefined): { value: number; unit: string } | null {
 ...
 */
function validateViewport(group: TokenGroup | undefined): void {
  if (!group) return;

  const mobile = parseLength(group.mobile?.value);
  const tablet = parseLength(group.tablet?.value);
  if (!mobile || !tablet) return;
  if (mobile.unit !== tablet.unit) return;
  if (tablet.value > mobile.value) return;

  throw new Error(
    `Config error: viewport.tablet (${group.tablet!.value}) must be larger than ` +
...
function validateTokenGroup(category: string, group: TokenGroup): void {
  for (const [key, entry] of Object.entries(group)) {
    if (category === 'viewport' && !(VIEWPORT_KEYS as readonly string[]).includes(key)) {
      throw new Error(
        `Config error: Token "viewport.${key}" is not a supported viewport key.\n` +
...
export function validateBaseStyles(
  ...
  const elements: Array<[string, BaseStyleElementDef | undefined]> = [
    ...
    ...ELEMENT_REGISTRY.map(e => [e.key, baseStyles[e.key]] as [string, BaseStyleElementDef | undefined]),
  ];
  ...
  if (padding) {
    for (const key of Object.keys(padding)) {
      if (!(ROOT_PADDING_KEYS as readonly string[]).includes(key)) {
        throw new Error(
          `Config error: baseStyles.spacing.padding.${key} is not a supported padding key.\n` +
