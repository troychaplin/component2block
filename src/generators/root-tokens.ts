 ...
 */
export function buildRootTokens(config: C2bConfig): Array<{ key: string; value: string }> {
 ...
 */
export function appendRootTokens(lines: string[], config: C2bConfig, separate: boolean): void {
  const rootTokens = buildRootTokens(config);
  if (rootTokens.length === 0) return;

  if (separate) lines.push('');
