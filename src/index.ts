...
export function generate(configPath?: string, cwd?: string): GenerateResult {
  ...
  const prefixed = (filename: string): string =>
    filename.startsWith('_') ? `_${config.prefix}-${filename.slice(1)}` : `${config.prefix}-${filename}`;
  ...
  const writeGenerated = (filename: string, content: string | null) => {
    if (!content) return;
